...
const HotPropositionCard: React.FC<HotPropositionCardProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        {/* Price Header */}
        <div className={`p-5 ${isSold ? 'bg-gray-100' : 'bg-gradient-to-br from-[#1c3664] via-[#1c3664] to-[#2a4a7a]'}`}>
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="text-left">
              <p className={`text-2xl font-black leading-tight ${isSold ? 'text-gray-400 line-through' : 'text-white'}`}>
                {price}
              </p>
            </div>
          </div>
        ...
        {/* Card Content */}
        <div className={`p-5 flex flex-col flex-1 justify-between ${isSold ? 'bg-gray-50' : 'bg-white'}`}>
          
          {/* Title and Location */}
          <div className="mb-4">
            <h3 className={`text-lg font-bold leading-tight line-clamp-2 mb-3 ${
          ...
          <div className="mt-auto pt-4 border-t border-gray-100">
            {isSold ? (
              <div className="flex items-center justify-center gap-2 px-5 py-3 bg-gray-300 text-white font-bold rounded-xl text-sm opacity-70">
                <CheckCircle2 size={16} />
                <span>נמכר</span>
              </div>
            ) : (
              <div className="flex items-center justify-center gap-2 px-5 py-3 bg-[#1c3664] text-white font-bold rounded-xl text-sm hover:bg-[#152a4f] transition-all duration-200 shadow-md hover:shadow-lg group-hover:scale-105">
