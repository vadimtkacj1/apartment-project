...
// Map raw API/server data (the /api/properties/[id] response shape) to the
// component-facing PropertyData. Pure — usable both for SSR-provided initial
// data and for client-side fetches.
function mapApiProperty(data: any): PropertyData {
...
export function usePropertyData(propertyId: string, initialData?: any) {
  // When the server already rendered with full data, map it synchronously so
  // the first render (including SSR HTML) shows the property — no loading state.
  const [property, setProperty] = useState<PropertyData | null>(() =>
    initialData ? mapApiProperty(initialData) : null
  );
  ...
  useEffect(() => {
    ...
    // Server provided the data — skip the client fetch, just record the view.
    if (initialData) {
    ...
    fetchProperty();
    // initialData is stable for the lifetime of the page (comes from server props)
