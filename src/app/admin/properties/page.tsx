...
function PropertiesPageInner() {
  ...
  const [listingOrderBusy, setListingOrderBusy] = useState(true);
  ...
  const fetchListingOrder = async () => {
    try {
      ...
      // Non-blocking: the rest of the page works, the control just shows the default
      console.error('Error fetching listing order:', error);
    } finally {
      setListingOrderBusy(false);
    }
  };
  ...
  const handleListingOrderChange = async (value: ListingOrder) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      toast.success(t.listingOrderSaved);
    } catch (error) {
      console.error('Error updating listing order:', error);
      toast.error(t.listingOrderError);
  ...
  return (
    <div>
      ...
      {/* Public listing order — a CMS setting, saved the moment it changes */}
      <Card className="mb-6 p-4">
        <div className="admin-filter-full flex w-full flex-col gap-3 sm:flex-row sm:items-center">
          <div className="min-w-0 sm:flex-1">
            <div className="text-sm font-semibold text-[#051150]">{t.listingOrderTitle}</div>
            <div className="mt-1 text-xs text-muted-foreground">{t.listingOrderHints[listingOrder]}</div>
          </div>
          <Select
            value={listingOrder}
            onValueChange={(v) => handleListingOrderChange(v as ListingOrder)}
            disabled={listingOrderBusy}
          >
            <SelectTrigger className="h-11 w-full sm:w-auto sm:min-w-[220px]" aria-label={t.listingOrderTitle}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {LISTING_ORDERS.map((order) => (
                <SelectItem key={order} value={order}>{LISTING_ORDER_LABELS[order]}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
