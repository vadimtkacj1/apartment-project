...
describe('admin owners', () => {
  ...
  it('GET blocks non-admins', async () => {
    ...
    expect((await ownerList(plainReq())).status).toBe(401);
    expect(prismaMock.owner.findMany).not.toHaveBeenCalled();
  });

  it('GET [id] blocks non-admins', async () => {
    requireAdmin.mockResolvedValue(denied);
    expect((await ownerGet(plainReq(), ctx('3'))).status).toBe(401);
...
describe('admin team members', () => {
  ...
  it('GET blocks non-admins', async () => {
    requireAdmin.mockResolvedValue(denied);
    expect((await teamList(plainReq())).status).toBe(401);
